...
void processInput(istream& input) {
    ...
    while (getline(input, userInput)) {
        ...
        if (getcwd(buffer, sizeof(buffer)) == NULL) {
            ...
            if(command == "mkdir"){
                //mkdir supports simultaneous directory creation (e.g., mkdir Saturn Mars Jupiter).
                //Use spaces to figure out where one folder name ends and the next begins.
                pathList = stringSplitter(userInput, ' ');
                //if a user types mkdir followed by no directory name, gently remind them to add one.
                if(pathList.size() == 1){
                    cout << "Please enter a directory name as well." << endl;
                }

                else{
                    //save the command name (mkdir) separately, so the list only holds directory names to check. 
                    string commandPart = pathList[0]; 
                    for(int i = 1; i < pathList.size(); i++){
                        // Clean up the folder name: remove extra spaces at the beginning or end (inside spaces are kept).
                        string dirName = trimString(pathList[i]);
                        // Skip over any blanks the user typed by mistake.
                        // So, mkdir      Parker == mkdir Parker
                        if(dirName.empty()){
                            continue;
                        }
                        else{
                            ...
                            const char* directoryName = dirName.c_str();
